...
import org.emu.docmanagment.config.KafkaProperties;
...
class DocumentmanagmentmsKafkaResourceIT {
    ...
    void setup() {
        ...
        DocumentmanagmentmsKafkaResource kafkaResource = new DocumentmanagmentmsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/documentmanagmentms-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/documentmanagmentms-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
